...
function getAllParagraphs() {
    return document.getElementById("myPage").getElementsByTagName("p");
}
...
function makeItalic() {
    const p = document.querySelectorAll(document.getElementsByClassName("even").getElementsByTagName("p"));
    p.forEach((p, index) => {
    p.italic();
    })
    return p;
...
function createBoarders() {
    const t = document.getElementById("myPage").getElementsByTagName("table");
    t.style.border = "2px thick solid #000";
    return t;
}
...
function getTheListItem(num1, num2) {
    return document.getElementById("myPage").getElementsByTagName('ul:nth-of-typeof(${num1})').getElementsByTagName('li:nth-of-typeof(${num2})');
}
...
function changeBgColor(color) {
    const b = document.getElementById("myPage");
